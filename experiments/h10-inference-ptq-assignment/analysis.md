...
## 2026-05-27 Final Instruct Marlin Artifact Result

The final artifact-backed PTQ path uses matched Llama-3.1-8B-Instruct
bf16/fp16 baselines rather than comparing Instruct artifacts against the
base-model H9.1 baseline. A clean final table was generated with only the
matched Instruct Marlin summaries:

```text
experiments/h10-inference-ptq-assignment/results/action_table_final_instruct_marlin.csv
experiments/h10-inference-ptq-assignment/results/selected_policy_final_instruct_marlin_strict.json
experiments/h10-inference-ptq-assignment/results/selected_policy_final_instruct_marlin_relaxed_3pct.json
```

The strict solver still uses the locked quality gate:

```text
predicted_quality_risk <= 0.01
```

Under that gate, `llama31_8b_instruct_gptq_marlin_artifact` is selected for all
three matched Instruct workloads:
...
This supports the H10 hypothesis in the strict setting: a backend-real
Marlin/GPTQ inference policy passes the 1% prompt-NLL gate and improves the
deployment frontier by substantially reducing latency and increasing output
throughput.

`llama31_8b_instruct_awq_marlin_artifact` is also backend-feasible and fast, but
it is rejected by the strict solver because its prompt-NLL delta is +2.851377%.
With a relaxed 3% sensitivity gate, AWQ-Marlin becomes accepted before Pareto
filtering and appears on the mixed-workload frontier, but that is a sensitivity
result rather than the main H10 claim.

The earlier combined table also contains artifact rows generated by the
one-command pipeline against the base-model H9.1 summary. Those rows are useful
